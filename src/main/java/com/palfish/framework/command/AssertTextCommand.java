...
import com.palfish.framework.utils.DataUtil;
...
public class AssertTextCommand implements Command {
    ...
    public AssertTextCommand(String arg) {
        this.arg = arg;
    }
    ...
    public void execute() {
        ...
        if(str.length == 3) {
            ...
            if(mobileElement != null) {
                Assert.assertEquals(mobileElement.getText(),str[2]);
            }
            ...
            if(Integer.valueOf(str[2]) <= list.size()) {
                ...
                if(mobileElement != null) {
                    Assert.assertEquals(mobileElement.getText(),str[3]);
                }
            } else {
